...
```{r, results = 'asis'}
cat(paste0("Witaj **", params$form$Imię, "** **",
           params$form$Nazwisko, "**!"))
```
...
if (paste(params$form$Imię, params$form$Nazwisko) %>%
  ...
  payment <- payment %>%
    mutate(
      ...
      `Rezygnacja z cateringu` = if_else(
        params$form$`Rezygnacja z cateringu` == "Tak",
        true = -10,
        false = 0
      )
    ) %>%
    mutate(`Suma [PLN]` = rowSums(.))
